...
function loadFolders() {
  $.ajax({
    type: 'GET',
    dataType: 'html',
    url: '{{ route("yunicot.lfm.getFolders") }}',
    data: {
...
function loadItems() {
  ...
  $.ajax({
    type: 'GET',
    dataType: 'html',
    url: '{{ route("yunicot.lfm.getItems") }}',
    data: {
...
function createFolder(folder_name) {
  $.ajax({
    type: 'GET',
    dataType: 'text',
    url: '{{ route("yunicot.lfm.getAddfolder") }}',
    data: {
      name: folder_name,
      working_dir: $('#working_dir').val(),
      type: $('#type').val()
    ...
    callback: function (result) {
      if (result !== null) {
        $.ajax({
          type: 'GET',
          dataType: 'text',
          url: '{{ route("yunicot.lfm.getRename") }}',
          data: {
            file: item_name,
            working_dir: $('#working_dir').val(),
            new_name: result,
  ...
  bootbox.confirm("{{ Lang::get('laravel-filemanager::lfm.message-delete') }}", function (result) {
    if (result == true) {
      $.ajax({
        type: 'GET',
        dataType: 'text',
        url: '{{ route("yunicot.lfm.getDelete") }}',
        data: {
          working_dir: $('#working_dir').val(),
          items: item_name,
          type: $('#type').val()
...
function resizeImage(image_name) {
  $.ajax({
    type: 'GET',
    dataType: 'text',
    url: '{{ route("yunicot.lfm.getResize") }}',
    data: {
      img: image_name,
      working_dir: $('#working_dir').val(),
      type: $('#type').val()
...
function download(file_name) {
  location.href = '{{ route("yunicot.lfm.getDownload") }}?'
  + 'working_dir='
  + $('#working_dir').val()
  + '&type='
  + $('#type').val()
